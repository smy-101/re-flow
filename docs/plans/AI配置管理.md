...
| 路由 | 方法 | 功能 | 认证 |
|------|------|------|------|
| `/api/ai-configs` | GET | 获取配置列表 | 需认证 |
| `/api/ai-configs` | POST | 创建配置 | 需认证 |
| `/api/ai-configs/[id]` | GET | 获取配置详情 | 需认证 |
| `/api/ai-configs/[id]` | PUT | 更新配置 | 需认证 |
| `/api/ai-configs/[id]` | DELETE | 删除配置 | 需认证 |
| `/api/ai-configs/[id]/test` | POST | 测试配置 | 需认证 |
  ...
  "configs": [
    {
      ...
      "apiKey": "sk-xxxx...xxxx", // 仅显示前4位和后4位
      "model": "gpt-4o",
...
3. 创建列表组件：`components/ai/AIConfigList.tsx`
4. 创建卡片组件：`components/ai/AIConfigCard.tsx`
5. 创建表单组件：`components/ai/AIConfigForm.tsx`
6. 创建模态框组件：`components/ai/AIConfigModal.tsx`
...
*创建日期: 2025-03-05*
*最后更新: 2025-03-05*
*更新内容: 新增第三方 AI 供应商支持，包括预置供应商配置和自定义供应商选项*
