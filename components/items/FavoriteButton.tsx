...
export default function FavoriteButton({
  ...
  return (
    <Button
      variant={currentIsFavorite ? 'primary' : 'secondary'}
      size="sm"
      onClick={handleToggle}
      loading={loading}
      title={currentIsFavorite ? '取消收藏' : '收藏'}
      className={currentIsFavorite ? 'bg-yellow-500 hover:bg-yellow-600 border-yellow-500' : ''}
    >
      {currentIsFavorite ? (
        <>
